...
def read_candles(path: Path) -> list[dict[str, Any]]:
    with path.open("r", encoding="utf-8", newline="") as handle:
        return [dict(row) for row in csv.DictReader(handle) if str(row.get("date") or "").strip()]


def eligibility_counts(candles: Iterable[dict[str, Any]], *, min_price: float, min_avg_dollar_volume: float, lookback_days: int) -> dict[str, Any]:
    rows = list(candles)
    ...
    return {"rows": len(rows), "checked_rows": checked, "eligible_rows": eligible, "five_day_returns_over_100pct": extreme_five_day_returns}


def audit(daily_dir: Path, symbols: list[str], *, min_price: float, min_avg_dollar_volume: float, lookback_days: int) -> dict[str, Any]:
    summaries = []
    missing = []
    for symbol in symbols:
        ...
        if not path.exists():
            ...
            continue
        counts = eligibility_counts(read_candles(path), min_price=min_price, min_avg_dollar_volume=min_avg_dollar_volume, lookback_days=lookback_days)
        if not counts["rows"]:
            missing.append(symbol)
            continue
    ...
    return {
        "status": "complete",
        "research_only": True,
        "policy": {"min_price": min_price, "min_avg_dollar_volume": min_avg_dollar_volume, "lookback_days": lookback_days},
...
def main() -> None:
    ...
    if args.min_price <= 0 or args.min_avg_dollar_volume <= 0 or args.lookback_days < 2:
        raise ValueError("price, dollar volume, and lookback must be positive; lookback must be at least two days")
    report = audit(args.daily_dir, load_symbols(args.symbols_file), min_price=args.min_price, min_avg_dollar_volume=args.min_avg_dollar_volume, lookback_days=args.lookback_days)
    args.output.parent.mkdir(parents=True, exist_ok=True)
    args.output.write_text(json.dumps(report, indent=2) + "\n", encoding="utf-8")
    print(json.dumps({key: report[key] for key in ("status", "symbols_with_history", "rows_checked", "rows_eligible", "five_day_returns_over_100pct")}, indent=2))
