...
- 2026-07-27: Corrected the contextual unusual-call portfolio risk measurement without deleting its conservative legacy metric. The prior cohort curve compounded each exit-date average as 100% account exposure. The new tested simulator marks every open position to market each day, limits each new position to one of five slots and available cash, leaves unused slots in cash, never borrows, and subtracts the stated cost. At the frozen top-quarter rule, all three existing chronological splits now pass the capital-scaled research gate: 40% split 59 signals, +2.439161% mean net, 55.93% wins, -10.280937% drawdown; 50% split 49 signals, +2.420582%, 57.14% wins, -10.280937%; 60% split 39 signals, +1.704715%, 51.28% wins, -13.930926%. These are retrospective robustness results from an already explored archive, not authorization for paper/live trading. The next requirement is the existing frozen prospective shadow protocol.
...
- 2026-07-27: Added a deterministic SHA-256 identity for every filename and byte of the 483-file Schwab daily archive used by the corrected portfolio simulator. Re-running all three frozen splits with daily archive hash `75b25e35e451...` reproduced the capital-scaled returns, drawdowns, and `RESEARCH_PASS` statuses exactly. This closes a report-provenance gap; it does not replace prospective validation.
