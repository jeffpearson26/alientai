...
- 2026-07-21: Added and tested a first lead-lag discovery tool using the existing Dow-30 daily library and SPY as a market control. It checks 1/2/3/5/10-session delays, controls for the target's current return, requires consistent direction over three chronological windows, and applies a conservative Bonferroni filter across 4,350 pair/lag tests. The strengthened initial pass generated 274 candidates from 30 symbols; that remains a research inventory, not a result or a signal. Approximate correlation p-values do not resolve serial dependence or economic value. Sector controls, rolling windows, effect-size/economic-value tests, and an untouched evaluation period remain required before any feature promotion.
