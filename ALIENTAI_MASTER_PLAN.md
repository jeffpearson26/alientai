...
- 2026-07-21: Added and tested a first lead-lag discovery tool using the existing Dow-30 daily library and SPY as a market control. It checks 1/2/3/5/10-session delays, controls for the target's current return, and requires consistent train/test direction. The initial pass generated 542 candidates from 30 symbols; that large discovery set is not a result or a signal. It must next receive sector controls, rolling windows, multiple-testing adjustment, effect-size/economic-value tests, and an untouched evaluation period before any feature promotion.
