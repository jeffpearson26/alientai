...
- 2026-07-23: Added and tested `audit_russell_liquidity_eligibility.py`, a research-only point-in-time preflight using a $5 minimum close and $5 million trailing-20-day average dollar-volume screen. Of 4,100,205 legacy-archive rows with sufficient lookback, 2,162,985 passed the preliminary liquidity screen; 1,688 five-day moves above +100% remain in the archive. This quantifies the need for a separate corporate-action/outlier policy and does not make the legacy list a point-in-time Russell universe or establish tradability/predictive value.
- 2026-07-23: Root-caused a critical legacy-history issue: some symbol files have long gaps between consecutive available rows (for example QUAD jumps from 2009-02-26 to 2010-07-05). A row-offset label could therefore misrepresent a multi-month/year change as a fixed trading-session horizon. Added and tested `audit_russell_price_discontinuities.py` to preserve such extreme events for review without altering source data. Updated the active S&P five-day LightGBM and 20-day Transformer training paths with a fail-closed continuity requirement: every lookback plus forecast window must have strictly increasing timestamps with no adjacent gap larger than five calendar days. New tests prove both paths reject a synthetic year-long gap. All existing legacy Russell model artifacts remain invalid for promotion and must be retrained from a clean, pre-registered universe only after the full data-integrity design is complete.
