...
def main():
    p=argparse.ArgumentParser(); p.add_argument("--target", required=True); p.add_argument("--symbols-jsonl",type=Path,required=True); p.add_argument("--chains",type=Path,required=True); p.add_argument("--output",type=Path,required=True); a=p.parse_args()
    rows=[]
    ...
    features=[row for row in unusual_call_features(rows) if row["market_date"]==a.target]
    a.output.parent.mkdir(parents=True,exist_ok=True)
    a.output.write_text("".join(json.dumps(row,sort_keys=True)+"\n" for row in features),encoding="utf-8")
    print(json.dumps({"status":"complete","target":a.target,"rows":len(features),"unusual":sum(bool(r.get('call_volume_unusual')) for r in features)},indent=2))
