...
"""Independently audit a daily-only technical and call-option panel."""
...
def audit(panel_root: Path) -> dict:
    ...
    if manifest.get("model_family") != "daily_options_cross_sectional_ranker":
        errors.append("unexpected model family")
    source = manifest.get("source_contract", {})
    for name in ("intraday", "afterhours", "news"):
    ...
    for name in [*DAILY_FEATURES, *OPTION_FEATURES]:
        column = f"rank_{name}"
