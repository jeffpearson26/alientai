...
`evaluate_context_portfolio.py --daily-dir data_v2\sp500_daily_schwab_max_history` now adds a daily mark-to-market, capital-scaled five-slot curve while retaining the old full-notional cohort approximation for auditability. It maps the archive's UTC candle-start date to the corresponding next-day US regular session, allocates at most one-fifth of marked equity and no more than cash, leaves unused slots in cash, never borrows, and charges the supplied round-trip cost on exit. At the already-frozen top-quarter cutoff, the capital-scaled research gate passes all three historical calibration splits: 40% (59 signals, -10.280937% drawdown), 50% (49, -10.280937%), and 60% (39, -13.930926%). Mean net trade returns were +2.439161%, +2.420582%, and +1.704715%, respectively. This corrects the risk diagnosis but does not make the explored archive prospective or authorize paper trading. Continue only through `FROZEN_CONTEXTUAL_OPTIONS_STUDY.md` and the non-executing prospective gate.
