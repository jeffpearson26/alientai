...
BUILD = "ALIENTAI_V2_PREDICTION_FRIDAY_SUPABASE_TRAINER_V1"
...
def normalize_row(row: Dict[str, Any]) -> Optional[Dict[str, Any]]:
    ...
    symbol = str(lower.get("symbol") or lower.get("ticker") or "").upper().strip()
    if not symbol:
        return None
...
def make_records_for_symbol(
    ...
    for i in range(min_history_days, len(rows)):
        ...
        # Entry must be before same week's final trading day.
...
def summarize_symbol(symbol: str, records: List[Dict[str, Any]], candles: int) -> Dict[str, Any]:
    total = len(records)
...
def fetch_symbol_rows(client, table: str, symbol: str, limit: int) -> List[Dict[str, Any]]:
    """
...
def fetch_all_symbols(client, table: str, max_symbols: int) -> List[str]:
    # Supabase paging.
    symbols = set()
    page_size = 1000
    start = 0
...
def main() -> None:
    ...
    parser.add_argument("--row-limit-per-symbol", type=int, default=10000)
    ...
    if args.only_symbol:
        ...
        symbols = fetch_all_symbols(client, args.table, args.max_symbols)
    ...
    print(f"Row limit per symbol: {args.row_limit_per_symbol}")
    ...
    with RECORDS_PATH.open("w", encoding="utf-8") as records_file:
        for idx, symbol in enumerate(symbols, start=1):
            print(f"[{idx}/{len(symbols)}] Training {symbol} from Supabase...")
            ...
            try:
                rows = fetch_symbol_rows(
                    ...
                    table=args.table,
                    symbol=symbol,
                    limit=args.row_limit_per_symbol,
                )
                ...
                summary = summarize_symbol(symbol, records, candles=len(rows))
    ...
    final = {
        ...
        "settings": {
            "row_limit_per_symbol": args.row_limit_per_symbol,
            "min_history_days": args.min_history_days,
